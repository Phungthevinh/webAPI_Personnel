...
namespace WebAPI.Controllers
{
    public class UsersControlers
    {
        ...
        //đăng ký người dùng
        public async Task<IResult> addUsers(role_user role_User)
        {
            try
            {
                string passwordHash = BCr.BCrypt.HashPassword(role_User.password_hash);
                ...
                _dbContext.Add(new Users
                {
                    ...
                    user_roles = new List<user_roles>
                    {
                        new user_roles
                        {
        ...
        public async Task<string> dangnhap(DangNhap dangnhap, string keyJWT, string Issuer, string Audience)
        {
            try
            {
               ...
               .FirstOrDefaultAsync();
                ...
                var user_role = await _dbContext.user_roles
                ...
                var role = await _dbContext.roles
                    .Where(u => u.id == user_role.role_id)
                    .Select(u => new
                    {
                        name = u.name
                    })
